    ...
    let dialog = new frappe.ui.Dialog({
        ...
        fields: [
            ...
            },
            {
                ...
                options: `
                    <div class="alert alert-info">
                        <p class="mb-1"><strong>${__('Import Process')}:</strong></p>
                        <ol class="mb-0">
                            <li>${__('Download Excel template')}</li>
                            <li>${__('Fill in your data')}</li>
                            <li>${__('Upload and validate file. If any information is incorrect, you must edit the file and upload it again.')}</li>
                            <li>${__('Import stock entries')}</li>
                        </ol>
                    </div>
                `
            },
            ...
            },
            {
                ...
                label: __('Template'),
                options: `
                    <p>${__('Download the Excel template with sample data and instructions.')}</p>
...
function validate_excel_file(values, dialog) {
    ...
    const progress_html = `
        <div class="validation-progress">
            ...
            <div class="text-center mt-2">
                ...
                <span id="validation-status">${__('Starting validation...')}</span>
            </div>
        </div>
    `;
    ...
    const progress_interval = setInterval(() => {
        ...
        if (progress > 30 && progress < 60) {
            $('#validation-status').text(__('Checking file format...'));
        } else if (progress >= 60) {
            $('#validation-status').text(__('Validating data...'));
        }
    }, 200);
    ...
    frappe.call({
        ...
        callback: function (r) {
            ...
            if (r.message) {
                ...
                dialog.set_value('validation_results', `<div class="alert alert-danger">${__('Validation failed')}</div>`);
            }
        },
        error: function (r) {
            clearInterval(progress_interval);
            dialog.set_value('validation_results', `<div class="alert alert-danger">${__('Error')}: ${r.message || __('Unknown error')}</div>`);
        }
    });
}
...
function display_validation_results(validation_result, dialog) {
    const create_success_html = () => `
        <div class="alert alert-success">
            <h5><i class="fa fa-check"></i> ${__('Validation Successful!')}</h5>
            <div class="row mt-2">
                <div class="col-md-4">
                    <div class="alert alert-info text-center py-3 mb-0">
                        <div class="h2 mb-1">${validation_result.total_rows}</div>
                        <div class="text-muted">${__('Total Rows')}</div>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="alert alert-success text-center py-3 mb-0">
                        <div class="h2 mb-1">${validation_result.valid_rows}</div>
                        <div class="text-muted">${__('Valid Rows')}</div>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="alert alert-primary text-center py-3 mb-0">
                        <div class="h2 mb-1">${validation_result.groups_count}</div>
                        <div class="text-muted">${__('Stock Entries')}</div>
                    </div>
                </div>
            </div>
        </div>
    ...
    const create_error_section = (title, items, formatter) => {
        ...
        if (display_items.length <= 5) {
            ...
            section += '<div style="max-height: 200px; overflow-y: auto;"><table class="table table-sm"><tbody>';
            display_items.forEach(item => {
        ...
        if (has_more) {
            section += `<small class="text-muted">... ${__('and {0} more items', [items.length - 10])}</small>`;
        }
    ...
    if (validation_result.success) {
        ...
        let html = `<div class="alert alert-danger"><h5><i class="fa fa-times"></i> ${__('Validation Failed')}</h5>`;

        html += create_error_section(__('Missing Items'), details.missing_items, item => {
            let result = `${__('Row {0}', [item.row])}: "${item.custom_item_name_detail}"`;
            if (item.suggestions && item.suggestions.length > 0) {
                const suggestion_names = item.suggestions.slice(0, 3).map(s => `"${s.custom_item_name_detail}"`).join(', ');
                result += `<br><small class="text-muted">${__('Suggestions')}: ${suggestion_names}</small>`;
            }
            return result;
        });

        html += create_error_section(__('Missing Warehouses'), details.missing_warehouses, warehouse =>
            `${__('Row {0}', [warehouse.row])}: ${warehouse.item_code} - ${warehouse.warehouse}`
        );

        html += create_error_section(__('Stock/Invoice Issues'), details.invoice_issues, issue =>
            `${__('Row {0}', [issue.row])}: ${issue.item_code} - ${__('Available')}: ${issue.available_qty}, ${__('Requested')}: ${issue.requested_qty}`
        );

        html += create_error_section(__('Other Errors'), details.errors, error => error);
...
function import_material_issue_entries(values, dialog) {
    ...
    const import_progress_html = `
        <div class="import-progress">
            ...
            <div class="text-center">
                ...
                <span id="import-status">${__('Preparing import...')}</span>
            </div>
            <div class="mt-2" id="import-details" style="display: none;">
                <small class="text-muted">${__('Processing groups')}: <span id="groups-processed">0</span> / <span id="total-groups">0</span></small>
            </div>
        </div>
    `;
    ...
    const import_interval = setInterval(() => {
        ...
        if (import_progress > 20 && import_progress < 50) {
            $('#import-status').text(__('Creating stock entries...'));
            $('#import-details').show();
        } else if (import_progress >= 50 && import_progress < 80) {
            $('#import-status').text(__('Validating stock movements...'));
        } else if (import_progress >= 80) {
            $('#import-status').text(__('Finalizing import...'));
        }
    }, 300);
        ...
        callback: function (r) {
            ...
            setTimeout(() => {
                if (r.message) {
                    ...
                    dialog.set_value('import_results', `<div class="alert alert-danger">${__('Import failed')}</div>`);
                }
            }, 500);
        },
        error: function (r) {
            clearInterval(import_interval);
            dialog.set_value('import_results', `<div class="alert alert-danger">${__('Error')}: ${r.message || __('Unknown error')}</div>`);
        }
    });
}
...
function display_import_results(import_result, dialog) {
    const create_success_summary = () => `
        <div class="alert alert-success">
            <h5><i class="fa fa-check-circle"></i> ${__('Import Completed Successfully!')}</h5>
            <div class="row mt-3">
                <div class="col-md-4">
                    <div class="alert alert-success text-center py-3 mb-0">
                        <div class="h2 mb-1">${import_result.success_count}</div>
                        <div class="text-muted">${__('Stock Entries Created')}</div>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="alert alert-info text-center py-3 mb-0">
                        <div class="h2 mb-1">${import_result.total_items}</div>
                        <div class="text-muted">${__('Items Processed')}</div>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="alert ${import_result.error_count > 0 ? 'alert-warning' : 'alert-success'} text-center py-3 mb-0">
                        <div class="h2 mb-1">${import_result.error_count}</div>
    ...
    const create_entries_list = () => {
        ...
        let entries_html = `<div class="mt-3">
            <h6>
                <i class="fa fa-list"></i> ${__('Created Stock Entries')}:
                <a href="/app/stock-entry?docstatus=0" target="_blank" class="btn btn-xs btn-primary ml-2" title="${__('Open all Draft Stock Entries')}">
                    <i class="fa fa-external-link"></i> ${__('View All Draft')}
                </a>
            </h6>
        </div>`;

        entries_html += '<div style="max-height: 400px; overflow-y: auto; border: 1px solid var(--border-color); border-radius: 4px;">';
        entries_html += `<table class="table table-sm table-hover table-striped mb-0">
            <thead>
                <tr>
                    <th style="width: 5%;">#</th>
        ...
        if (has_more) {
            entries_html += `<div class="mt-2">
                <small class="text-muted">${__('Showing {0} of {1} entries', [display_entries.length, entries_details.length])}</small>
            </div>`;
        }
    ...
    const create_errors_list = () => {
        ...
        let errors_html = `<div class="mt-3"><h6 class="text-danger"><i class="fa fa-exclamation-triangle"></i> ${__('Errors')}:</h6>`;
        ...
        if (has_more) {
            errors_html += `<small class="text-muted d-block mt-2">... ${__('and {0} more errors', [import_result.errors.length - 10])}</small>`;
        }
    ...
    if (import_result.success_count > 0) {
        ...
        html += `<div class="alert alert-danger"><h5><i class="fa fa-times-circle"></i> ${__('Import Failed')}</h5></div>`;
    }
    ...
    if (import_result.success_count > 0) {
        if (dialog.listview) dialog.listview.refresh();
        frappe.show_alert({
            message: __('Successfully created {0} Stock Entries', [import_result.success_count]),
            indicator: 'green'
        });
    }
}
...
function validate_receipt_excel_file(values, dialog) {
    ...
    const progress_html = `
        <div class="validation-progress">
            ...
            <div class="text-center mt-2">
                <i class="fa fa-spinner fa-spin"></i>
                <span id="receipt-validation-status">${__('Starting validation...')}</span>
            </div>
        </div>
    `;
    dialog.set_value('validation_results', progress_html);
    ...
    const progress_interval = setInterval(() => {
        ...
        if (progress > 30 && progress < 60) {
            $('#receipt-validation-status').text(__('Checking file format...'));
        } else if (progress >= 60) {
            $('#receipt-validation-status').text(__('Validating data...'));
        }
    }, 200);
...
function import_material_receipt_entries(values, dialog) {
    ...
    const import_progress_html = `
        <div class="import-progress">
            ...
            <div class="text-center">
                <i class="fa fa-cog fa-spin"></i>
                <span id="receipt-import-status">${__('Preparing import...')}</span>
            </div>
            <div class="mt-2" id="receipt-import-details" style="display: none;">
                <small class="text-muted">${__('Processing groups')}: <span id="receipt-groups-processed">0</span> / <span id="receipt-total-groups">0</span></small>
            </div>
        </div>
    `;
    dialog.set_value('import_results', import_progress_html);
    ...
    const import_interval = setInterval(() => {
        ...
        if (import_progress > 20 && import_progress < 50) {
            $('#receipt-import-status').text(__('Creating stock entries...'));
            $('#receipt-import-details').show();
        } else if (import_progress >= 50 && import_progress < 80) {
            $('#receipt-import-status').text(__('Validating stock movements...'));
        } else if (import_progress >= 80) {
            $('#receipt-import-status').text(__('Finalizing import...'));
        }
    }, 300);
