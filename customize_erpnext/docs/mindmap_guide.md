# Hướng dẫn bộ sơ đồ mindmap — Sơ đồ chức năng hệ thống / System Functional Mindmaps

> **Mục đích:** Tài liệu **giới thiệu và hướng dẫn người dùng**: hệ thống có những chức năng gì, mỗi chức năng dùng để làm gì, chức năng nào là chuẩn / đã sửa / phát triển thêm, và tiến độ đến đâu.
> **Phạm vi:** Tài liệu
...
```bash
cd apps/customize_erpnext/customize_erpnext/scripts
...
- API chỉ đọc/ghi file `.md` **trong `docs/mindmap`**: `os.path.realpath` rồi kiểm tra prefix, nên `../hooks.py` hay `/etc/passwd` đều bị chặn.
...
node scripts/tests/test_mindmap.js
```
