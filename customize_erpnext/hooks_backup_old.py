...
fixtures = [ 
    ...
    # Custom Workspace - DISABLED due to missing required 'type' field in v16
    # {
    #     "doctype": "Workspace",
    #     "filters": [
    ...
    ,
    # Workspace customizations (sidebar modifications)
    {
        "doctype": "Workspace",
        "filters": [
            ["name", "in", [
                "HR",  # Contains Shift & Attendance
                "Stock"
            ]]
        ]
    },
    # Workspace Sidebar - Export sidebar customizations (v16+)
    # NOTE: ERPNext v16 không còn cơ chế "extends" như v15
    # Phải export TOÀN BỘ sidebar structure, không chỉ custom items
    # Khi import sẽ OVERWRITE hoàn toàn sidebar gốc
    {
    ...
    # Custom Reports - QUAN TRỌNG: Export cùng với Workspace Sidebar
    #
    # WHY NEEDED:
    # - Workspace Sidebar chỉ export LINKS/REFERENCES đến reports (label, icon, position)
    # - Report fixtures export ĐỊNH NGHĨA ĐẦY ĐỦ của report (metadata, permissions, settings)
    # - Nếu thiếu Report fixtures:
    #   ✗ Sidebar có link nhưng report không tồn tại trên site đích
    #   ✗ User click vào sẽ gặp lỗi "Report not found" hoặc "Permission denied"
    #
    # WHAT IS EXPORTED:
    # - Report metadata (name, type, ref_doctype, module)
    # - Permissions (roles có quyền xem report)
    # - Settings (timeout, add_total_row, prepared_report, etc.)
    #
    # NOTE: Script Report code (Python/JS) không được export trong fixtures
    #       Code đã có sẵn trong app folder và sync qua git
    #
    # RELATIONSHIP:
    # Workspace Sidebar fixtures = Bookmark/Shortcut
    # Report fixtures           = Application được bookmark
    # → Cần CẢ 2 để sidebar links hoạt động!
    {
        "doctype": "Report",
        "filters": [
            ["name", "in", [
                "Shift Attendance Customize",   # Added to "Shift & Attendance" sidebar
                "Overtime Registration",        # Added to "Shift & Attendance" sidebar
                "Stock Ledger Customize",       # Added to "Stock" sidebar
                "Stock Balance Customize",      # Added to "Stock" sidebar
            ]]
        ]
    }
    # ,
