...
> Cập nhật: 2026-06-29 (thuật toán app cũ) / 2026-07-05 (§15)
...
| Mục trong tài liệu | Triển khai ERPNext |
|---|---|
| §2 Shift params | Shift Type doctype (start/end + custom break fields). KHÔNG có effective-dating — đổi giờ ca lịch sử phải tạo Shift Type mới + Shift Assignment |
| §2b TimesheetSettings | Single DocType **Attendance Calculation Setting** (`/app/attendance-calculation-setting`). ĐÃ CHỐT: minOtMinute = **30**, GIỮ THÊM min riêng 60' cho OT trước ca (khác tài liệu); otBlock/workingBlock mặc định **1** |
| §4 Xác định ca | Shift Assignment → Employee default_shift → setting `default_shift`. ĐÃ CHỐT: CN dùng ca được phân của NV (KHÔNG ép Day / nghỉ trưa 12-13 cố định — dùng break config của ca) |
| §5 Thai sản | Employee Maternity doctype (3 giai đoạn tường minh); benefit hours = setting |
